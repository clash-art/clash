...
export function registerDocumentTools(
  ...
  options: { request: DocumentRequest },
): void {
  const client = createDocumentClient(options.request);
  server.registerTool(
    "clash_assets_document_revision_get",
    {
      title: "Read Document Asset revision",
      description: describeClashTool({
        useWhen:
          "an Output Commit or pinned input names a Document Asset revision whose body is needed",
        effect:
          "reads the exact immutable revision through the Project Document API without changing its head",
        returns:
          "the revision, its producer and source references, and its verified body",
    ...
    async ({ projectId, documentAssetId, revisionId }) => {
      const value = publicDocumentValue(
        await client.getRevision(projectId, documentAssetId, revisionId),
      );
      return {
